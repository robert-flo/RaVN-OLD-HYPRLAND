...
def get_hyprctl_binds():
    # Try the JSON output first (documented form: `hyprctl -j binds`).
    # In Hyprland 0.56 the "binds -j" (and sometimes -j first) emitted
    # corrupted non-JSON (wrong types, unquoted idents like "Q", shifted values).
    ...
    # Fallback: parse the stable plain-text `hyprctl binds` output.
    # This format has never been broken and contains all needed info.
    try:
        result = subprocess.run(
            ["hyprctl", "binds"],
            capture_output=True,
...
def parse_hyprctl_binds_text(text: str):
    """Parse `hyprctl binds` (plain) into the dict shape the rest of the
    script (and old -j output) expects.
    """
    binds = []
    current = None
    for line in text.splitlines():
        ...
        if key == "modmask":
            current["modmask"] = int(val) if val else 0
        elif key == "submap":
            current["submap"] = val
        elif key == "key":
            current["key"] = val
            ...
            try:
                current["keycode"] = int(val)
            except ValueError:
                current["keycode"] = 0
        elif key == "catchall":
            current["catch_all"] = val.lower() == "true"
        elif key == "description":
            current["description"] = val
            current["has_description"] = val.startswith("[")
        elif key == "dispatcher":
            current["dispatcher"] = val
        elif key == "arg":
            current["arg"] = val
    if current:
        binds.append(current)
