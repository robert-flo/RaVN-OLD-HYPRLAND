...
# -o : Restaura configuraciones de usuario y fuerza la sobreescritura sin sudo.
...
# El modo -o es una restauración autónoma de recursos bajo $HOME. Mantiene -ro
# como alias compatible y evita que combinaciones como -io activen fases de
# instalación privilegiadas.
...
export flg_DryRun flg_Nvidia flg_Shell flg_Install flg_Restore flg_ThemeInstall RAVN_LOG flg_Overwrite flg_user_only
...
cleanup() {
  ...
  if ((flg_Install == 1))   && [[ -f $scrDir/install_pkg.lst ]]; then
...
if [ "${flg_user_only}" -eq 1 ]; then
  print_log -g "[user-only] " -b "enabled :: " "Restoring user configuration without sudo"
fi
...
if ((flg_DryRun == 0 && flg_user_only == 0)); then
...
if [ ${flg_Restore} -eq 1 ] && [ ${flg_user_only} -eq 0 ]; then
...
# ==============================================================================
if [ ${flg_Restore} -eq 1 ] && [ ${flg_user_only} -eq 0 ] && [ -f "${scrDir}/launchers/install_launchers.sh" ]; then
  cat << "EOF"
...
if ((flg_user_only == 0 && flg_DryRun != 1))   &&
  ((flg_Install == 1 || flg_Restore == 1 || flg_Service == 1)); then
