...
ravn_launcher_install 'Codex TUI' 'sh -c '\''exec xdg-terminal-exec --app-id=TUI.tile -e codex --dangerously-bypass-approvals-and-sandbox'\''' 'Codex.svg' '--comment=Codex CLI — AI coding agent (bypass approvals)' '--categories=ConsoleOnly;TUI;Utility;' '--keywords=tui;terminal;cli;codex;ai;'
