...
#
# # USO BÁSICO
#   git-setup.sh              # Menú interactivo
#   git-setup.sh <comando>    # Modo directo
#
# # SUBCOMANDOS
#
#   verify | -v   — Verifica la configuración actual sin modificar nada
#   setup  | -s   — Configura desde cero: gh auth, SSH, GPG, signing
...
#   |-------------------------------|-------------------------------|------------------------------------------------|
#   | Verificar solo                | git-setup.sh -v               | Estado de todo sin cambios                     |
#   | Setup completo                | git-setup.sh -s               | GitHub SSH GPG config desde cero               |
#   | Test rápido (local)           | git-setup.sh -t               | Crea repo, prueba signing, no push             |
#   | Test con push                 | git-setup.sh test             | Test + pregunta si hacer push                  |
#   | Clean (con confirmación)      | git-setup.sh -c               | Pide "yes" antes de borrar                     |
#   | Menú interactivo              | git-setup.sh                  | Navegación por números                         |
#   | Ayuda rápida                  | git-setup.sh help             | Lista de comandos                              |
#
# # FLUJO RECOMENDADO
#   1. git-setup.sh verify     # Ver qué falta
#   2. git-setup.sh setup      # Configurar todo
#   3. git-setup.sh test       # Probar que funciona
...
configure_signing() {
  ...
  print_success "Default branch set to: ${WHITE}main${NC}"
}
...
run_test() {
  ...
  # Test 2: Create file and stage
  {
    ...
    echo ""
    echo "This is a test repository created by git-setup.sh"
    echo "Created: $(date)"
  } >README.md
...
show_menu() {
  ...
  echo -e "    ${CYAN}4${NC}  󰃢  Clean local config (fresh start)"
