...
check_scheduled_actions() {
  ...
  scheduled_action=$(get_config "scheduled_action")
  local scheduled_time=""
  scheduled_time=$(get_config "scheduled_time")
  local last_checked=""
  last_checked=$(get_config "last_checked_timestamp")

  if [[ "$scheduled_action" != "none" && "$scheduled_time" != "none" ]]; then
    local current_ts=""
    current_ts=$(date +%s)
    local scheduled_ts_today=""
...
generate_waybar_output() {
  ...
  local task_count=""
  task_count=$(echo "$all_tasks_json" | jq 'length')
