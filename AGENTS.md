...
- **Required agent validation for `Configs/` changes:** After changing any file beneath `Configs/`, run `bash Scripts/install.sh -o` and verify the affected live target in `$HOME`. `-o` is the user-only overwrite mode: it must not request or execute `sudo`, intentionally skips system resources and dependency installation, and reloads HyDE after a successful restore. If `hyde-shell` is unavailable it warns and continues. Use `bash Scripts/install.sh -ot` to preview the same restoration and reload without changing files or the desktop session. Use `install.sh -r` only when the full privileged restoration workflow is intended.
...
- **Mandatory branch-baseline preflight**: Before creating a normal feature or chore branch from `dev`, first run `git fetch origin --prune` and verify `git merge-base --is-ancestor origin/master origin/dev`. A zero exit status means the released history is already in `dev`. If it fails, update `dev` with `git merge --ff-only origin/master` from a clean `dev` worktree, then push `dev`, fetch again, and repeat the ancestry check. This advances the branch reference without creating a direct commit or merge commit. If fast-forward is impossible, do not force it or derive new work: use an isolated synchronization branch and PR to resolve the divergence first. This prevents features from missing released behavior.
...
- **`dev`**: The active branch for all features and PRs. **Under no circumstances should `dev` receive direct commits.** It must always be fed exclusively by auxiliary topic/feature branches created in isolated worktrees (via `git-create-worktree`, in `Configs/.local/bin/` — see "Repository Structure & Purpose") and merged in. The sole exception is the documented baseline preflight: a clean `dev` worktree may fast-forward directly to `origin/master` with `git merge --ff-only origin/master` and push that existing history; it creates no commit and must not be used for feature work.
